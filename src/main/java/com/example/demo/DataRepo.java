...
public interface DataRepo extends JpaRepository<Data, DataId> {
    ...
    @Query("SELECT d.location FROM Data d WHERE d.id.vNumber = ?1 AND d.date = ?2 AND d.vState = 1")
    ...
    @Query("SELECT COUNT(d) FROM Data d WHERE d.id.vNumber = ?1 AND d.date = ?2 AND d.vState = 1")
    ...
    @Query("SELECT COUNT(d) FROM Data d WHERE d.id.vNumber = ?1 AND d.date = ?2 AND d.vState = 0")
